...
- **One scheduled frame for camera and hover updates.** An idle map performs no rendering. Wheel bursts use the latest camera state. Trackpad scrolling pans on both axes; a mouse wheel zooms around the pointer. Trackpad pinch/Ctrl + wheel zooms independently of device detection; Shift + wheel explicitly pans. Pixel, line and page deltas are normalized, with a cap on unusually large ordinary-wheel steps. Resizing preserves the camera. Locking a path preserves zoom and the inspected node's screen position, including any navigation while the path request is pending.
...
This is heuristic detection: browsers expose [no standard mouse-versus-trackpad identifier in WheelEvent](https://lists.w3.org/Archives/Public/public-pointer-events/2026JanMar/0031.html). Some smooth-scrolling mice and trackpads produce indistinguishable events. The manual setting makes those cases deterministic without affecting phone touch gestures or desktop pinch.
